    ...
    for col in df.columns:
        ...
        if null_count > 0:
            if df[col].dtype == 'object':
                ...
                log.append(f"Column '{col}': filled {null_count} missing values with mode '{fill_val}'")
...
"""
        response = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "You are a professional Data Analyst. Write clean reports in plain English. Never use asterisks, hashtags, bullet symbols, or any markdown. Use numbered sections and plain paragraphs only."},
...
"""
        viz_response = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "You are a Data Visualization Expert. Write clean reports in plain English. Never use asterisks, hashtags, bullet symbols, or markdown. Use numbered sections and plain paragraphs only."},
...
def download_pdf():
    ...
    try:
        ...
        log_style = ParagraphStyle(
            'LogStyle', parent=styles['Normal'],
            fontSize=9, spaceAfter=4, leftIndent=20,
